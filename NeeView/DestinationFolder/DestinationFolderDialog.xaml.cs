...
namespace NeeView
{
    ...
    public partial class DestinationFolderDialog : Window
    {
        ...
        private void ItemsListView_PreviewDragOver(object sender, DragEventArgs e)
        {
            ...
            {
                e.Effects = e.Data.GetNormalizedFileDrop().Any(x => FileIO.DirectoryExists(x)) ? DragDropEffects.Copy : DragDropEffects.None;
                e.Handled = true;
            }
        }
        ...
        private void ItemsListView_Drop(object sender, DragEventArgs e)
        {
            if (e.Data.GetDataPresent(DataFormats.FileDrop, true))
            {
                foreach (var path in e.Data.GetNormalizedFileDrop().Where(x => FileIO.DirectoryExists(x)))
                {
                    _vm.Add(path);
                }
                e.Handled = true;
